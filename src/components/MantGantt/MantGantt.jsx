...
export default function MantGantt() {
    ...
    const { tasks: tasksWithStatus, categories: mantCategories, categoryColors, loading, refetch } = useGanttData(DEPT_NAME);
    const {
    ...
    const baseCols = 15;
    const totalCols = isAdmin ? baseCols + 1 : baseCols;
    ...
    return (
        <div className="mgantt">
            <div className="mgantt__filters">
                ...
                {hasFilters && (<button className="mgantt__clear-all" onClick={() => { setSemaforoFilter(null); setCategoryFilter(null); }}><FiX /> Limpiar filtros</button>)}
            </div>
            ...
            <div className="mgantt__wrapper">
                <table className="mgantt__table">
                    <thead><tr>
                        ...
                        {isAdmin && <th className="mgantt__th gantt__th--actions"></th>}
                    </tr></thead>
                    <tbody>
                        {mantCategories.map((cat) => {
                                    ...
                                    {catTasks.map((task, idx) => {
                                        ...
                                        return (
                                            <tr key={task.id} className={`mgantt__row ${hoveredTask === task.id ? 'mgantt__row--hover' : ''}`}
                                                ...
                                                <td className="mgantt__td mgantt__td--task"><span className="mgantt__task-name">{task.name}</span>{task.notes && <span className="mgantt__task-note">{task.notes}</span>}</td>
                                                ...
                                                <td className="mgantt__td mgantt__td--sem">
                                                    {(() => {
                                                        ...
                                                        return (<span className="mgantt__sem-pill" style={{ '--s-color': ec, '--s-bg': `${ec}18`, cursor: isAdmin ? 'pointer' : 'default' }}
                                                ...
                                                {isAdmin && (<td className="mgantt__td gantt__td--actions"><button className="gantt__delete-btn" onClick={() => handleDeleteClick(task)} title="Eliminar tarea"><FiTrash2 /></button></td>)}
                                            </tr>
                                        );
                                    })}
                                </Fragment>
