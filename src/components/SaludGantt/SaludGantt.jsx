...
export default function SaludGantt() {
    ...
    const { tasks: tasksWithStatus, categories: saludCategories, categoryColors, loading, refetch } = useGanttData(DEPT_NAME);
    const {
                    ...
                    <div className="sgantt__filter-buttons">
                        {['green', 'yellow', 'red'].map((s) => (
                            <button key={s} className={`sgantt__filter-btn ${semaforoFilter === s ? 'sgantt__filter-btn--active' : ''}`}
                                ...
                                <span className="sgantt__filter-dot" /><span>{statusLabels[s]}</span>
            ...
            <div className="sgantt__wrapper">
                <table className="sgantt__table">
                    <thead><tr>
                        ...
                        {isAdmin && <th className="sgantt__th gantt__th--actions"></th>}
                    </tr></thead>
                    <tbody>
                        {saludCategories.map((cat) => {
                                ...
                                <Fragment key={cat}>
                                    ...
                                    {catTasks.map((task, idx) => (
                                        <tr key={task.id} className={`sgantt__row ${hoveredTask === task.id ? 'sgantt__row--hover' : ''}`}
                                            ...
                                            <td className="sgantt__td sgantt__td--task"><span className="sgantt__task-name">{task.name}</span>{task.note && <span className="sgantt__task-note">{task.note}</span>}</td>
                                            <td className="sgantt__td sgantt__td--sem">
                                                {(() => {
                                                    ...
                                                    return (<span className="sgantt__sem-pill" style={{ '--s-color': ec, '--s-bg': `${ec}18`, cursor: isAdmin ? 'pointer' : 'default' }}
                                            ...
                                            {isAdmin && (<td className="sgantt__td gantt__td--actions"><button className="gantt__delete-btn" onClick={() => handleDeleteClick(task)} title="Eliminar tarea"><FiTrash2 /></button></td>)}
                                        </tr>
                                    ))}
                                </Fragment>
                            );
