...
export default function CrecGantt() {
    ...
    const { tasks: tasksWithStatus, categories: crecCategories, categoryColors, loading, refetch } = useGanttData(DEPT_NAME);
    const {
        isAdmin, canCreate,
    ...
    return (
        <div className="crgantt">
            <div className="crgantt__filters">
                ...
                {hasFilters && (<button className="crgantt__clear-all" onClick={() => { setSemaforoFilter(null); setCategoryFilter(null); }}><FiX /> Limpiar filtros</button>)}
            </div>
            ...
            <div className="crgantt__wrapper">
                <table className="crgantt__table">
                    <thead><tr>
                        ...
                        {isAdmin && <th className="crgantt__th gantt__th--actions"></th>}
                    </tr></thead>
                    <tbody>
                        {crecCategories.map((cat) => {
                            ...
                            return (
                                <Fragment key={cat}>
                                    <tr className="crgantt__cat-header"><td colSpan={isAdmin ? 15 : 14} className="crgantt__cat-header-cell" style={{ '--cat-color': cc }}>
                                    ...
                                    {tasks.map((task, idx) => (
                                        <tr key={task.id} className={`crgantt__row ${hoveredTask === task.id ? 'crgantt__row--hover' : ''}`}
                                            ...
                                            <td className="crgantt__td crgantt__td--task">
                                                <span className="crgantt__task-name">{task.name}</span>
                                            ...
                                            <td className="crgantt__td crgantt__td--sem">
                                                {(() => {
                                                    ...
                                                    return (<span className="crgantt__sem-pill" style={{ '--s-color': ec, '--s-bg': `${ec}18`, cursor: isAdmin ? 'pointer' : 'default' }}
                                            ...
                                            {isAdmin && (<td className="crgantt__td gantt__td--actions"><button className="gantt__delete-btn" onClick={() => handleDeleteClick(task)} title="Eliminar tarea"><FiTrash2 /></button></td>)}
                                        </tr>
                                    ))}
                                </Fragment>
                            );
                        })}
                        {filtered.length === 0 && (<tr><td colSpan={isAdmin ? 15 : 14} className="crgantt__empty"><span className="crgantt__empty-icon">🔍</span><p>No hay actividades con estos filtros</p></td></tr>)}
                    </tbody>
                </table>
            </div>
