...
export default function GestionGantt() {
    ...
    const { tasks: tasksWithStatus, categories: gestionCategories, categoryColors, departments, loading, refetch } = useGanttData(DEPT_NAME);
    const {
        isAdmin, canCreate,
        ...
        <div className="ggantt">
            <div className="ggantt__filters">
                <div className="ggantt__filters-row">
                    ...
                    <button className={`ggantt__filter-btn ${unreadFilter ? 'ggantt__filter-btn--active' : ''}`}
                        style={{ '--f-color': '#ef4444', '--f-bg': '#ef444418' }}
                        onClick={() => setUnreadFilter((v) => !v)}>
                        <FiMessageSquare size={12} /><span>No leídas</span>
                        <span className="ggantt__filter-count">{unreadCount}</span>
                    </button>
                </div>
                <div className="ggantt__filters-row">
                ...
                {hasFilters && (<button className="ggantt__clear-all" onClick={() => { setSemaforoFilter(null); setCategoryFilter(null); setUnreadFilter(false); }}><FiX /> Limpiar filtros</button>)}
            </div>
            ...
            <div className="gantt__toolbar">
                ...
                <div className="gantt__toolbar-actions">
                    {isAdmin && (
                        <button className="grid-print-btn" onClick={() => printTable(tableRef.current, 'Gestión de Asociados')}>
                            <FiPrinter size={14} /> Imprimir
                        </button>
                    )}
                    {isAdmin && (
                        <button className="grid-print-btn" onClick={() => exportCSV(filtered, 'Gestion_de_Asociados', getDeptStatus)}>
                        ...
                        <div className="ggantt__wrapper">
                            <table className="ggantt__table">
                                <thead><tr>
                                    ...
                                    <th className="ggantt__th ggantt__th--actions"></th>
                                </tr></thead>
                                <tbody>
                                    {gestionCategories.map((cat) => {
                                        ...
                                        return (
                                            <Fragment key={cat}>
                                                <tr className="ggantt__cat-header"><td colSpan={15} className="ggantt__cat-header-cell" style={{ '--cat-color': cc }}>
                                                ...
                                                {tasks.map((task, idx) => (
                                                    <tr key={task.id} className={`ggantt__row ${hoveredTask === task.id ? 'ggantt__row--hover' : ''}`}
                                                        ...
                                                        <td className="ggantt__td ggantt__td--task">
                                                            <span className="ggantt__task-name" style={{ cursor: 'pointer' }} onClick={() => setDetailTask(task)} title="Ver detalle">{task.name}{(task.observations?.length > 0) && <ObservationIndicator activityId={task.id} count={task.observations.length} />}</span>
                                                        ...
                                                        <td className="ggantt__td ggantt__td--sem">
                                                            {(() => {
                                                                ...
                                                                return (<span className="ggantt__sem-pill" style={{ '--s-color': ec, '--s-bg': `${ec}18`, cursor: 'default' }}>
                                    ...
                                    {filtered.length === 0 && (<tr><td colSpan={15} className="ggantt__empty"><span className="ggantt__empty-icon">🔍</span><p>No hay actividades con estos filtros</p></td></tr>)}
                                </tbody>
                            </table>
                        </div>
